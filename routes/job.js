...
router.get("/search", async (req, res) => {
  ...
  const options = {
    ...
    // fieldNormWeight: 1,
    keys: ["designation", "job_responsibilities", "other_info", "job_field"],
  };
  ...
  if (req.query.t) {
    ...
    list = list.filter((el) => {
      const types = el.item.job_type.split(",").map((type) => type.trim());
      ...
      if (type === "any") {
        ...
        return criteriaArray.some((criteria) => types.includes(criteria));
      }
    });
  }
  ...
  if (req.query.loc) {
    ...
    list = list.filter((el) => {
      return (
        el.item.company.state === state && el.item.company.country === country
      );
    });
  }
  ...
  const uniqueLocations = [
    ...new Set(
      list.map((el) => el.item.company.state + ", " + el.item.company.country)
    ),
  ];
...
router.get("/company/:company_id", async (req, res) => {
  ...
  res.status(200).json({
    ...
    status: "200",
    message:
      "List of jobs for company " + company_id + " successfully retrieved.",
    data: {
      jobList,
      uniqueField,
    },
